...
public class MovieDialog extends JDialog {
    ...
    private final JList<String> lstGenre;

    private final JTable tabCharacters;

    private String outcome = "cancel";

	private final MovieDialogCallback callback;
	private TabModelCharacter tabModel;

	private MovieDTO movie;
	
	public MovieDialog(Window owner, MovieDialogCallback callback, List<String> allGenres) {
		super(owner);
		...
		gbc.gridx = 0;
		gbc.gridy = 4;
        JLabel labGenre = new JLabel("Genres:");
		...
		gbc.gridheight = 4;
		lstGenre = new JList<>();
        JScrollPane scrGenre = new JScrollPane(lstGenre);
        ...
        JScrollPane scrCharacters = new JScrollPane(tabCharacters);
		...
		btnUp.addActionListener(e -> moveCharacterUp());
		...
		btnDown.addActionListener(e -> moveCharacterDown());
		...
		btnAdd.addActionListener(e -> addCharacter());
		...
		btnEdit.addActionListener(e -> editCharacter());
		...
		btnDelete.addActionListener(e -> deleteCharacter());
	...
	public void editCharacter() {
		...
		if (row != -1) {  // es gibt eine Selektion
